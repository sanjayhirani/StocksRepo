...
def create_exact_infographic(ticker, row, info, fin, cashflow):
    try:
        ...
        # --- HEADER ---
        ax.text(0.5, 13.0, ticker, fontsize=90, fontweight='black', color='#ff4b4b')
        ax.text(9.5, 13.2, f"${safe_float(info.get('marketCap'))/1e9:.1f}B Market Cap", ha='right', fontsize=22, fontweight='bold')
        ax.text(9.5, 12.6, f"{row['5Y_Perf']:.0f}% 5Y", ha='right', fontsize=20, fontweight='bold', color=GREEN if row['5Y_Perf']>0 else RED)
        ax.text(9.5, 12.1, f"{row['YTD_Perf']:.0f}% YTD", ha='right', fontsize=20, fontweight='bold', color=GREEN if row['YTD_Perf']>0 else RED)

        # --- FINANCIAL BARS ---
        ax.text(3.5, 11.2, "● Revenue  ● Net Income  ● FCF", fontsize=10, color='#666666')
        try:
            df_fin = fin.T if fin.shape[0] < fin.shape[1] else fin
            df_cf = cashflow.T if cashflow.shape[0] < cashflow.shape[1] else cashflow
            dp = pd.DataFrame({
                'R': df_fin.get('Total Revenue', 0),
                'N': df_fin.get('Net Income', 0),
                'F': df_cf.get('Free Cash Flow', df_cf.get('Operating Cash Flow', 0) + df_cf.get('Capital Expenditure', 0))
            }).fillna(0).head(10)[::-1]
            if not dp.empty:
                ...
                for i, (idx, v) in enumerate(dp.iterrows()):
                    ...
                    ax.add_patch(Rectangle((x_pts[i]-0.07, 0.15), (v['N']/norm)*2.2, color=TEAL)) # Fixed syntax
                    ax.add_patch(Rectangle((x_pts[i]+0.08, 8.5), 0.15, (v['F']/norm)*2.2, color='#ff4b4b'))
        ...
        fcf_val = safe_float(info.get('freeCashflow', info.get('operatingCashflow', 0) + info.get('capitalExpenditures', 0)))
        draw_ring(ax, 8.1, 8.0, (fcf_val/rev)*100 if rev != 0 else 0, "FCF", RED)
        ...
        draw_ring(ax, 0.7, 6.0, 107, "Net Retention", RED)
        draw_ring(ax, 0.7, 5.2, safe_float(info.get('returnOnAssets'))*100, "ROIC", TEAL)
        n_cash = (safe_float(info.get('totalCash')) - safe_float(info.get('totalDebt'))) / 1e9
        ax.text(0.5, 4.3, f"• ${n_cash:.1f}B Net Cash", fontsize=15, fontweight='bold')
        ax.text(0.5, 3.8, f"• {safe_float(info.get('forwardPE')):.0f} P/E 12 Forward", fontsize=15, fontweight='bold')

        # --- FAIR VALUE BAR ---
        ax.add_patch(FancyBboxPatch((4.1, 4.2), 1.8, 0.5, boxstyle="round,pad=0.1", color=TEAL))
        ax.add_patch(FancyBboxPatch((6.0, 4.2), 1.0, 0.5, boxstyle="round,pad=0.1", color=RED))
        ax.text(5.5, 4.8, "Fair Value Bar", fontweight='bold', ha='center', fontsize=10)

        # --- PRICE TAG (ORANGE) ---
        tag_pts = [[7.5, 6.5], [9.8, 6.5], [9.8, 0.4], [7.9, 0.4], [7.5, 3.5]]
        ax.add_patch(Polygon(tag_pts, color=ORANGE_TAG))
        ax.add_patch(Circle((7.8, 3.5), 0.15, color=BG_WHITE))
        ax.text(8.7, 5.3, "Price", ha='center', fontsize=20, fontweight='bold')
        ...
        ax.text(8.7, 3.4, f"{((t_mean/row.Price)-1)*100:.0f}% OFF", ha='center', fontweight='bold', bbox=dict(facecolor='white', edgecolor='none'))
        ax.text(8.7, 0.8, f"${t_mean:.0f} Consensus", ha='center', fontweight='bold')

        # --- FOOTER ---
        ax.text(0.5, 0.5, "Global Equity Briefing\n23. March 2026", fontsize=9, color='#777777')
...
def run_scanner():
    ...
    # --- GOOGLE SHEETS UPDATES (FULLY RESTORED) ---
    for s_name in ["Core Screener", "Summary"]:
        ws = sh.worksheet(s_name)
        export_df = df_full.copy() if s_name == "Core Screener" else df_full.head(10).copy()
        data_to_push = [export_df.columns.tolist()] + export_df.astype(str).values.tolist()
        ws.clear()
    ...
    for i, (idx, r) in enumerate(df_full.head(5).iterrows()):
        ...
        img = create_exact_infographic(r.Stock, r, to.info, to.financials, to.cashflow)
        if img:
