...
def create_exact_infographic(ticker, row, info, fin):
    try:
        ...
        ax.text(9.5, 13.2, f"${m_cap:.1f}B Market Cap", ha='right', fontsize=18, fontweight='bold')
        ...
        # --- 3-COL BAR CHART ---
        # Map common yfinance financial names to our chart
        mapping = {'Total Revenue': BLACK, 'Net Income': TEAL, 'Free Cash Flow': RED}
        if not fin.empty:
            # Transpose if necessary and select available columns
            df_fin = fin.T if fin.shape[0] < fin.shape[1] else fin
            available_cols = [c for c in mapping.keys() if c in df_fin.columns]
            
            if available_cols:
                df_chart = (df_fin[available_cols].head(6)[::-1] / 1e6).fillna(0)
                x_pos = np.linspace(0.8, 6.5, len(df_chart))
                width = 0.12
                norm = df_chart.iloc[:, 0].max() if df_chart.iloc[:, 0].max() > 0 else 1
                ...
                for i, (idx, vals) in enumerate(df_chart.iterrows()):
                    ...
                    year_label = str(idx.year) if hasattr(idx, 'year') else str(idx)[:4]
                    ax.text(x_pos[i], 8.2, year_label, ha='center', fontsize=8, color='#666666')
        ...
        ax.text(0.5, 5.3, f"• ${info.get('totalCash', 0)/1e9:.1f}B Cash", fontsize=11)
        ax.text(0.5, 4.8, f"• {info.get('trailingPE', 0):.1f} P/E", fontsize=11)
        ...
        # Growth Since 2022
        ax.text(0.5, 3.8, "Growth Since 2022", fontsize=16, fontweight='bold', color=GREEN)
        draw_ring(ax, 0.7, 3.1, row.RS_Rating, "Revenue", TEAL, size=0.15)
        draw_ring(ax, 0.7, 2.5, row.RVOL*50, "EPS", TEAL, size=0.15)
